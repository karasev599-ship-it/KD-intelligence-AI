...
(function(){
  ...
  function loadEpochModule(){return new Promise((resolve,reject)=>{if(window.__kdE2EEEpochsInstalled){resolve();return;}if(window.__kdInstallE2EEEpochs&&window.__kdInstallE2EEEpochs()){resolve();return;}const existing=document.querySelector('script[data-kd-e2ee-epochs]');if(existing){let n=0;const t=setInterval(()=>{if(window.__kdE2EEEpochsInstalled){clearInterval(t);resolve();}else if(++n>50){clearInterval(t);reject(Error('E2EE epoch module timeout'));}},100);return;}const s=document.createElement('script');s.src='./e2ee-epochs.js';s.dataset.kdE2eeEpochs='1';s.onload=()=>{if(window.__kdInstallE2EEEpochs&&window.__kdInstallE2EEEpochs()){resolve();}else if(window.__kdE2EEEpochsInstalled)resolve();else reject(Error('E2EE epoch installer unavailable'));};s.onerror=()=>reject(Error('E2EE epoch module failed to load'));document.head.appendChild(s);});}
  ...
  async function decryptVisible(){
    ...
    try{await loadEpochModule();}catch(e){return;}
