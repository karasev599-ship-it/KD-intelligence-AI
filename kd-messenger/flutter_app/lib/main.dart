...
class Chat {
  Chat(this.name, this.handle, this.avatar, this.lastMessage, this.time, {this.online = false});
...
class _MessengerHomeState extends State<MessengerHome> {
  ...
  final chats = <Chat>[
    ...
    Chat('KD Team', '@kdteam', 'KD', 'Новое сообщение', 'Вчера'),
  ];
  ...
  void sendMessage() {
    ...
    setState(() {
      messages.add(Message(text, true, _time()));
      controller.clear();
    });
  }
      ...
      body: SafeArea(
        child: LayoutBuilder(
          builder: (context, constraints) {
            ...
            return ChatList(chats: chats, selected: selected, onSelect: (c) => _openMobileChat(c));
          },
        ),
      ),
    );
  ...
  void _openMobileChat(Chat chat) {
    Navigator.of(context).push(MaterialPageRoute(
      builder: (_) => ConversationPage(chat: chat, messages: messages, controller: controller, onSend: sendMessage),
    ));
  }
}
...
class ChatList extends StatelessWidget {
  ...
  final Chat? selected;
  final ValueChanged<Chat> onSelect;
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Padding(
          ...
          child: Row(children: [
            ...
            const Expanded(child: Text('KD Messenger', style: TextStyle(fontSize: 21, fontWeight: FontWeight.w800))),
        ...
        Padding(
          ...
          child: TextField(
            decoration: InputDecoration(hintText: 'Поиск чатов', prefixIcon: const Icon(Icons.search), filled: true, fillColor: const Color(0xFF15151D), border: OutlineInputBorder(borderRadius: BorderRadius.circular(15), borderSide: BorderSide.none)),
          ),
        ),
            ...
            itemBuilder: (_, i) {
              ...
              return InkWell(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(color: active ? const Color(0xFF1B1928) : Colors.transparent, borderRadius: BorderRadius.circular(14)),
                  child: Row(children: [
                    Stack(children: [
                      ...
                      if (c.online) Positioned(right: 0, bottom: 1, child: Container(width: 13, height: 13, decoration: BoxDecoration(color: const Color(0xFF55D889), shape: BoxShape.circle, border: Border.all(color: const Color(0xFF08080D), width: 2)))),
                    ]),
                    const SizedBox(width: 12),
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(c.name, style: const TextStyle(fontWeight: FontWeight.w700)), const SizedBox(height: 4), Text(c.lastMessage, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white54, fontSize: 13))])),
