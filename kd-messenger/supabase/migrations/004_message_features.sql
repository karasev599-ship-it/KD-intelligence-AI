...
create policy if not exists kd_reactions_member_read on public.kd_message_reactions
  for select to authenticated using (
    exists (select 1 from public.kd_messages x where x.id = message_id and public.kd_is_member(x.conversation_id))
  );
create policy if not exists kd_reactions_self_insert on public.kd_message_reactions
...
create policy if not exists kd_reactions_self_delete on public.kd_message_reactions
  for delete to authenticated using (user_id = auth.uid());

create policy if not exists kd_messages_sender_delete on public.kd_messages
