...
function buildInput(message, history, context) { const safeHistory = Array.isArray(history) ? history.slice(-16) : []; const transcript = safeHistory.map(m => `${m?.role === "assistant" ? "Assistant" : "User"}: ${String(m?.content || "").slice(0, 5000)}`).join("\n\n"); return [{role:"developer",content:SYSTEM_PROMPT},{role:"developer",content:`APPLICATION CONTEXT (JSON)\n${JSON.stringify(context || {}, null, 2)}`},...(transcript?[{role:"developer",content:`RECENT CHAT HISTORY\n${transcript}`}]:[]),{role:"user",content:String(message || "").slice(0,8000)}]; }
function localCoachReply(message, context = {}) { const q = String(message || '').toLowerCase(); const trades = Array.isArray(context?.trades) ? context.trades : []; const count = trades.length; const wins = trades.filter(t => String(t?.result || t?.status || '').toLowerCase().includes('win') || Number(t?.pnl) > 0).length; const losses = trades.filter(t => String(t?.result || t?.status || '').toLowerCase().includes('loss') || Number(t?.pnl) < 0).length; const pnl = trades.reduce((sum, t) => sum + (Number.isFinite(Number(t?.pnl)) ? Number(t.pnl) : 0), 0); if (/привет|здравств|кто ты|что ты умеешь|как дела/.test(q)) return 'Привет 👋 Я KriptoDanik AI Coach. Я могу бесплатно разбирать базовые вопросы по трейдингу, риск-менеджменту, психологии и твоему Journal. Когда шлюз OpenAI будет оплачен и доступен, свободные вопросы будут дополнительно обрабатываться реальной моделью.'; if (/проанализ|сделк|journal|журнал/.test(q)) { if (!count) return 'В Journal пока нет сделок для анализа. Добавь хотя бы одну — тогда я смогу разобрать результат, Win Rate, P&L и дисциплину по имеющимся данным.'; const winRate = count ? Math.round((wins / count) * 100) : 0; return `Быстрый разбор Journal:\n\n• Сделок: ${count}\n• Плюсовых: ${wins}\n• Минусовых: ${losses}\n• Win Rate по доступным данным: ${winRate}%\n• Суммарный P&L: ${pnl >= 0 ? '+' : ''}${pnl.toFixed(2)}\n\nЭто базовая статистика. Для глубокого разбора сетапов, психологии и закономерностей нужен доступ к полной истории и реальной AI-модели.`; } if (/ошиб|ошибк|mistake/.test(q)) return 'Быстрая проверка ошибок: сначала смотри на нарушения собственных правил, размер риска, наличие Stop Loss, вход без подтверждения и сделки после серии убытков. Самая полезная ошибка — та, которую можно превратить в конкретное правило на следующую сделку.'; if (/психолог|эмоци|страх|тильт|tilt/.test(q)) return 'Быстрый разбор психологии: не пытайся убрать эмоции полностью. Цель — не дать им менять исполнение системы. Перед входом проверь сетап, риск и подтверждение; после двух последовательных убытков остановись, если это предусмотрено твоими правилами.'; if (/риск|risk|плеч|leverage|размер позици/.test(q)) return 'Базовый риск-менеджмент: сначала определяй допустимый риск на сделку, затем расстояние до Stop Loss и только после этого размер позиции. Плечо не должно определять допустимый риск — оно лишь меняет требуемую маржу и чувствительность позиции.'; if (/рекоменда|совет|что делать|дай совет/.test(q)) return 'Моя базовая рекомендация сейчас: не усложняй систему без статистического основания. Фиксируй сделки одинаково, соблюдай заранее заданный риск и анализируй серии результатов, а не одну случайную сделку.'; return 'Я пока работаю в бесплатном локальном режиме. Могу ответить на базовые вопросы по трейдингу, риску, психологии и Journal. Для полноценного свободного диалога и более глубокого анализа после подключения оплаченного OpenAI-шлюза этот же вопрос автоматически уйдёт в реальную модель.'; }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!(await rateLimit(req, 'ai_coach', 30, 60, user.id))) return rateLimited(res, 60);
    const body = typeof req.body === "string" ? JSON.parse(req.body) : (req.body || {});
    const message = String(body.message || "").trim();
    if (!message) return json(res, 400, { error: "Message is required." });
    if (message.length > 8000) return json(res, 413, { error: "Message is too long." });
    if (!process.env.OPENAI_API_KEY) return json(res, 200, { reply: localCoachReply(message, body.context), model: "local", webSearch: false, local: true });
    ...
    if (!upstream.ok) { console.warn("OpenAI unavailable, using local Coach fallback:", data?.error?.message || upstream.status); return json(res, 200, { reply: localCoachReply(message, body.context), model: "local", webSearch: false, local: true, gatewayError: true }); }
  ...
  } catch(error) { console.error("AI Coach gateway error:",error); try { const body = typeof req.body === "string" ? JSON.parse(req.body) : (req.body || {}); return json(res,200,{reply:localCoachReply(body.message,body.context),model:"local",webSearch:false,local:true}); } catch (_) { return json(res,500,{error:"AI Coach gateway failed."}); } }
}
