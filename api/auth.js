...
function safeSignatureEqual(a,b) { const aa=Buffer.from(String(a||'')); const bb=Buffer.from(String(b||'')); return aa.length===bb.length&&crypto.timingSafeEqual(aa,bb); }
...
function parseCookies(req) { const out={}; String(req.headers.cookie||'').split(';').forEach(part=>{const i=part.indexOf('=');if(i<0)return;try{out[part.slice(0,i).trim()]=decodeURIComponent(part.slice(i+1).trim())}catch{}});return out; }
function bodyTooLarge(req) { const n=Number(req.headers['content-length']||0); return Number.isFinite(n)&&n>MAX_BODY_BYTES; }
function readBody(req) { const raw=typeof req.body==='string'?req.body:JSON.stringify(req.body||{}); if(Buffer.byteLength(raw,'utf8')>MAX_BODY_BYTES) throw Object.assign(new Error('Request body is too large.'),{status:413}); return typeof req.body==='string'?JSON.parse(req.body):(req.body||{}); }
...
async function currentUser(req) {
  ...
  if(!safeSignatureEqual(sign(payload),sig))return null;
  const [id,exp]=payload.split('.');if(!id||!/^\d+$/.test(exp)||Number(exp)<Math.floor(Date.now()/1000))return null;
